...
## sm-imageclass-patch
	thumbstat=$(cat $inputfile|grep 'thumbstat:'|sed -e 's/.*://'|sed -e "s/'//g")
	if [ "$freezelines" -gt 0 ]
	then
		while read -r line
		do
...
## sm-imageclass-patch
	if [ "$(echo "$freezepercent"|cut -d"." -f1)" -gt "$freezethreshold" ] || [ "$thumbstat" == "distort" ]
	then
		newstat="WARNING"
	fi
fi
...
then
## sm-imageclass-patch
	echo "          <THUMBCLASS>$thumbstat</THUMBCLASS>"
